...
namespace HtmlRenderer.Skia.MyAdapters
{
    /// <summary>
    ...
    public sealed class SKHtmlContainer : IDisposable
    {
        ...
        /// Init.
        /// </summary>
        public SKHtmlContainer()
        {
            _htmlContainerInt = new HtmlContainerInt(SKCanvasAdapter.Instance);
        ...
        public void PerformPaint(SKCanvas g,SKRect clip)
        {
            ArgChecker.AssertArgNotNull(g, "g");
            if(clip == SKRect.Empty)
            {
                using (var ig = new SKGraphicsAdapter(g))
                {
            ...
            using (var ig = new SKGraphicsAdapter(g, Utils.Convert(clip)))
            {
                _htmlContainerInt.PerformPaint(ig);
            }
        }
