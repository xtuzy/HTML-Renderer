...
namespace HtmlRendererCore.Skia.Maui
{
    ...
    public static class HtmlRender
    {
        ...
        /// <returns></returns>
        public static void AddFont(string path, string fontFamily)
        {
            path = path.ToLowerInvariant();

            if (CustomFontFamilyManager.GetFont(fontFamily) != null)
                return;

            var typeface = SKTypeface.FromFile(path);
            CustomFontFamilyManager.AddFont(typeface, fontFamily);
        }
        ...
        /// <returns></returns>
        public static async void AddMauiFont(string fontFullName, string fontFamily)
        {
            if (CustomFontFamilyManager.GetFont(fontFamily) != null)
                return;

            var typeface = SKTypeface.FromStream(await FileSystem.OpenAppPackageFileAsync(fontFullName));
            CustomFontFamilyManager.AddFont(typeface, fontFamily);
        }
        #endregion
        ...
        public static void AddFontFamily(string fontFamily)
        {
            ...
            SKCanvasAdapter.Instance.AddFontFamily(new SKPaintAdapter(fontFamily));
        }
        ...
        public static SKSize Measure(string html, double maxWidth = 0, CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            SKSize actualSize = SKSize.Empty;
            if (!string.IsNullOrEmpty(html))
            {
                using (var container = new HtmlContainer())
                {
                    container.MaxSize = new SKSize((float)maxWidth, 0);
        ...
        public static SKSize Render(SKCanvas g, string html, double left = 0, double top = 0, double maxWidth = 0, CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            ArgChecker.AssertArgNotNull(g, "g");
            return RenderClip(g, html, new SKPoint((float)left, (float)top), new SKSize((float)maxWidth, 0), cssData, stylesheetLoad, imageLoad);
        }
        ...
        /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
        /// <returns>the actual size of the rendered html</returns>
        public static SKSize Render(SKCanvas g, string html, SKPoint location, SKSize maxSize, CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            ArgChecker.AssertArgNotNull(g, "g");
        ...
        public static SKBitmap RenderToImage(string html, SKSize size, CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            //var renderTarget = new RenderTargetBitmap((int)size.Width, (int)size.Height, 96, 96, PixelFormats.Pbgra32);
            var renderTarget = new SKBitmap((int)size.Width, (int)size.Height, SKColorType.Rgba8888, SKAlphaType.Unknown);
            if (!string.IsNullOrEmpty(html))
            {
                // render HTML into the visual
                //DrawingVisual drawingVisual = new DrawingVisual();
                using (SKCanvas g = new SKCanvas(renderTarget))
                {
                    RenderHtml(g, html, new SKPoint(), size, cssData, stylesheetLoad, imageLoad);
                }
        ...
        /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
        /// <returns>the generated image of the html</returns>
        public static SKBitmap RenderToImage(string html, int maxWidth = 0, int maxHeight = 0, SKColor backgroundColor = new SKColor(), CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            return RenderToImage(html, SKSize.Empty, new SKSize(maxWidth, maxHeight), backgroundColor, cssData, stylesheetLoad, imageLoad);
        }
        ...
        /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
        /// <returns>the generated image of the html</returns>
        public static SKBitmap RenderToImage(string html, SKSize minSize, SKSize maxSize, SKColor backgroundColor = new SKColor(), CssData cssData = null,
            EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad = null, EventHandler<HtmlImageLoadEventArgs> imageLoad = null)
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                using (var container = new HtmlContainer())
                {
                    ...
                    //renderTarget = new SKBitmap((int)finalSize.Width, (int)finalSize.Height, 96, 96, PixelFormats.Pbgra32);
                    renderTarget = new SKBitmap((int)finalSize.Width, (int)finalSize.Height, SKColorType.Rgba8888, SKAlphaType.Unknown);

                    // render HTML into the visual
                    //DrawingVisual drawingVisual = new DrawingVisual();
                    using (SKCanvas g = new SKCanvas(renderTarget))
                    {
                        container.PerformPaint(g, SKRect.Create(new SKSize(maxSize.Width > 0 ? maxSize.Width : float.MaxValue, maxSize.Height > 0 ? maxSize.Height : float.MaxValue)));
                    }
            ...
            {
                //renderTarget = new SKBitmap(0, 0, 96, 96, PixelFormats.Pbgra32);
                renderTarget = new SKBitmap(0, 0, SKColorType.Rgba8888, SKAlphaType.Unknown);
            }
        ...
        private static SKSize MeasureHtmlByRestrictions(HtmlContainer htmlContainer, SKSize minSize, SKSize maxSize)
        {
            ...
            using (var mg = new SKGraphicsAdapter())
            {
                ...
                if (maxSize.Width < 1 && sizeInt.Width > 4096)
                    sizeInt.Width = 4096;
                return Skia.Maui.Utilities.Utils.ConvertRound(sizeInt);
            }
        }
        ...
        /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
        /// <returns>the actual size of the rendered html</returns>
        private static SKSize RenderClip(SKCanvas g, string html, SKPoint location, SKSize maxSize, CssData cssData, EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad, EventHandler<HtmlImageLoadEventArgs> imageLoad)
        {
            if (maxSize.Height > 0)
            {
                g.Save();
                g.ClipRect(SKRect.Create(location, maxSize));
            }
        ...
        /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
        /// <returns>the actual size of the rendered html</returns>
        private static SKSize RenderHtml(SKCanvas g, string html, SKPoint location, SKSize maxSize, CssData cssData, EventHandler<HtmlStylesheetLoadEventArgs> stylesheetLoad, EventHandler<HtmlImageLoadEventArgs> imageLoad)
        {
            SKSize actualSize = SKSize.Empty;
            ...
            if (!string.IsNullOrEmpty(html))
            {
                using (var container = new HtmlContainer())
                {
                    ...
                    container.SetHtml(html, cssData);
                    container.PerformLayout();
                    container.PerformPaint(g, new SKRect(0, 0, float.MaxValue, float.MaxValue));
